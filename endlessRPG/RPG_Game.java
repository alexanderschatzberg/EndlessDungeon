...
public class RPG_Game {
	...
	int baseSwingDamage = 5, baseStabDamage = 6, baseQuickStabDamage = 8, baseGiantSwingDamage = 15, baseZapDamage = 7,
			baseLightningDamage = 15, baseBasicFireballDamage = 9;
	...
	public void start() {
		...
		playerOptions.add(
				new OptionButton("Slash", "Stam: -" + 8 * baseSwingDamage, 100, 200, 80, 100, 2, "rpgImages/slash.png", 0, 0));
		playerOptions.add(
				new OptionButton("Stab", "Stam: -" + 8 * baseStabDamage, 220, 200, 80, 100, 3, "rpgImages/stab.png", 0, 0));
		playerOptions.add(
				new OptionButton("Zap", "Mana: -" + baseZapDamage * 8, 340, 200, 80, 100, 9, 1, "rpgImages/zap1.png", 80, 40));
		...
		allOptions.add(new OptionButton("Lightning", "Mana: -" + baseLightningDamage * 8, 580, 200, 80, 100, 9, 2,
				"rpgImages/lightning1.png", 0, 0));
		allOptions.add(new OptionButton("Fireball", "Mana: -" + baseBasicFireballDamage * 8, 580, 200, 80, 100, 9, 0,
				"rpgImages/basicFireball1.png", 0, 0));
		allOptions.add(new OptionButton("Giant Slash", "Stam: -" + 20 * baseGiantSwingDamage, 580, 200, 80, 100, 10,
				"rpgImages/giantSlash.png", 0, 0));
		allOptions.add(new OptionButton("Quick Stab", "Stam: -" + 12 * baseQuickStabDamage, 580, 200, 80, 100, 11,
				"rpgImages/quickStab.png", 0, 0));
	...
	RPG_Game() {
		...
		soundTimer.schedule(new BackgroundMusicTask(), 0, (3 * 60 + 13) * 1000); // Schedule the background music to play
																																							// every 3 minutes and 13
		...
		canvas.addMouseListener(new MouseListener() {
			...
			public void mousePressed(MouseEvent e) {
				...
				// If a player option is clicked, change the necessary variables to represent
				// that change
				if (playerOptionsVisible && !paused && !selectSkill) {
					for (OptionButton option : playerOptions) {
						if (option.isOn(e)) {
							...
							if (option.isSpell())
								spellChoice = option.getSpellChoice();
	...
	// Changes the variables that control animation based on where the game is in
	// its sequence
	// Runs the game's logic
	private void logic() {

		// Reset all indexing variables and prepares for a new scene if the scene
		// changes
		if (scene != lastScene) {
			count = 0;
			goal = 0;
	...
	private void animate() {
		...
		// Player slashes
			...
			// If the action is complete, have the player idle, monster take damage, and
		...
		// Player performs a giant slash
			...
			// If the action is complete, have the player idle, monster take damage, change
			// to next scene
		...
		// Player meditates
			...
			// If the action is complete, have the player idle, regain mana, change to next
			// scene
			if (count == 0) {
				goal = manaRegen;
			}
		...
		if (playerSpell != null) {
			...
			if (!spell_first_stage) {
				...
				// If the action is complete, have the monster idle, monster take damage, change
				// to next scene, spell is null
				if (playerSpell.explode()) {
					...
					monster.takeDamage(
							(int) ((playerSpell.getDamage() * statMenu.getIntelligence() * crit * intelligenceMultiplier / 2
									+ statMenu.getWisdom() * intelligenceMultiplier) * coefficient));
		...
		if (monster != null) {
			...
			if (monsterAction == 1) {
				...
				// If the action is complete, have the monster idle, player hurts, player takes
				// damage
				if (monster.attack1()) {
					monsterAction = 0;
					playerAction = 5;
	...
	private void monsterSpawn() {
		...
		if (choice.equals("SnowMonster"))
			monster = new SnowMonster(WIDTH, animationDelay, player.getX(), HEIGHT, monsterHomeX, targetLevel,
					countToDetermineIfBoss > 5, player.getLevel());
		if (choice.equals("Goblin"))
			monster = new Goblin(WIDTH, animationDelay, player.getX(), HEIGHT, monsterHomeX, targetLevel,
					countToDetermineIfBoss > 5, player.getLevel());
		if (choice.equals("Slime"))
			monster = new Slime(WIDTH, animationDelay, player.getX(), HEIGHT, monsterHomeX, targetLevel,
					countToDetermineIfBoss > 5, player.getLevel());
	...
	// This updates the player's damage values and bars to reflect stats and game
	// play
	private void updatePlayer() {
		...
		if (Math.random() > 0.6) {
			for (int i = 0; i < statMenu.getLuck() * 0.2; i++) {
				if (Math.random() * 10 * Math.pow(100, i / 2) < statMenu.getLuck()
						* (int) (1 + 0.2 * statMenu.getIntelligence() * intelligenceMultiplier)
						* (int) (1 + 0.1 * statMenu.getWisdom() * wisdomMultiplier))
					crit *= 2;
			}
		}

		// Determines damage values
		swingDamage = crit * strengthMultiplier * baseSwingDamage * statMenu.getStrength()
				+ baseSwingDamage * (int) (1 + 0.2 * statMenu.getWisdom() * wisdomMultiplier);
		stabDamage = crit * strengthMultiplier * baseStabDamage * statMenu.getStrength()
				* (int) (1 + 0.2 * statMenu.getWisdom() * wisdomMultiplier) / 10;
		quickStabDamage = crit * strengthMultiplier * baseQuickStabDamage * statMenu.getStrength()
				+ baseSwingDamage * (int) (1 + 0.4 * statMenu.getWisdom() * wisdomMultiplier);
		giantSwingDamage = crit * strengthMultiplier * baseGiantSwingDamage * statMenu.getStrength() + baseSwingDamage
				* (int) (1 + 0.2 * statMenu.getWisdom() * wisdomMultiplier) * (int) (1 + 0.2 * statMenu.getConstitution());

		// Determines player health
		player.setCurrentHealth(
				player.getCurrentHealth() + statMenu.getConstitution() * 10 * constitutionMultiplier - player.getTotalHealth());
		player.setTotalHealth(statMenu.getConstitution() * 10 * constitutionMultiplier);

		// Determines player stamina
		curStamina = curStamina + statMenu.getConstitution() * 5 * constitutionMultiplier + statMenu.getStrength() * 5
				- totalStamina;
		totalStamina = statMenu.getConstitution() * 5 * constitutionMultiplier + statMenu.getStrength() * 5;
		staminaRegen = statMenu.getConstitution() / 2 * constitutionMultiplier
				+ statMenu.getStrength() / 5 * strengthMultiplier;

		// Determines player mana
		curMana = curMana + statMenu.getIntelligence() * 5 * intelligenceMultiplier
				+ statMenu.getIntelligence() * wisdomMultiplier - totalMana;
	...
	private void playerLevelUp() {
		...
		curXP = overflowXP;
		statMenu.setStatPoints(statMenu.getStatPoints() + 8);
	...
	class RunSoundTask extends TimerTask {
		@Override
		public void run() {
			if ((playerAction == 1 || playerAction == 8) && !paused && !end)
				runSound(); // Plays the run sound
		}
	}
}
